        ...
        return $table
            ...
            ->actions([
                ...
                Tables\Actions\Action::make('update_kyc')
                    ...
                    ->action(function (ProcessRegistrationBonus $processRegistrationBonus, KycVerification $record, array $data) {
                        $reason = match ($data['status']) {
                            KYCStatus::Rejected->value => $data['comment'],
                            default => ''
                        };

                        if ($data['status'] === KYCStatus::Completed->value) {
                            $processRegistrationBonus->execute($record->user);
                        }
